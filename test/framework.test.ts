...
describe("Integration Tests:", () => {
  ...
  describe("Routing & Request Handling:", () => {
    it("should work for async controller methods", (done) => {
      ...
      supertest(server.build()).get("/").expect(200, "GET", done);
    });
    ...
    it("should work for async controller methods that fails", (done) => {
      ...
      server = new InversifyExpressServer(container);
      supertest(server.build()).get("/").expect(500, done);
    });
    ...
    it("should work for each shortcut decorator", (done) => {
      ...
      class TestController {
        ...
        @httpPut("/") public putTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("PUT");
        }
        @httpPatch("/") public patchTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("PATCH");
        }
        @httpHead("/") public headTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("HEAD");
        }
        @httpDelete("/") public deleteTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("DELETE");
      ...
      let deleteFn = () => {
        agent.delete("/").expect(200, "DELETE", done);
      };
      let head = () => {
        agent.head("/").expect(200, "HEAD", deleteFn);
      };
      let patch = () => {
        agent.patch("/").expect(200, "PATCH", head);
      };
      let put = () => {
        agent.put("/").expect(200, "PUT", patch);
      };
      let post = () => {
        agent.post("/").expect(200, "POST", put);
      };
      let get = () => {
        agent.get("/").expect(200, "GET", post);
      };
    ...
    it("should work for more obscure HTTP methods using the httpMethod decorator", (done) => {
      @controller("/")
      class TestController {
        @httpMethod("propfind", "/") public getTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("PROPFIND");
      ...
      server = new InversifyExpressServer(container);
      supertest(server.build()).propfind("/").expect(200, "PROPFIND", done);
    });
    ...
    it("should work for controller methods who's return value is falsey", (done) => {
      ...
      server = new InversifyExpressServer(container);
      supertest(server.build()).delete("/user").expect(204, "", done);
    });
  });
  ...
  describe("Middleware:", () => {
    ...
    it("should call method-level middleware correctly (GET)", (done) => {
      @controller("/")
      class TestController {
        @httpGet("/", spyA, spyB, spyC) public getTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("GET");
      ...
      agent.get("/").expect(200, "GET", function () {
    ...
    it("should call method-level middleware correctly (POST)", (done) => {
      @controller("/")
      class TestController {
        @httpPost("/", spyA, spyB, spyC) public postTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("POST");
      ...
      agent.post("/").expect(200, "POST", function () {
        expect(spyA.calledOnce).to.eqls(true);
        expect(spyB.calledOnce).to.eqls(true);
        expect(spyC.calledOnce).to.eqls(true);
        expect(result).to.equal("abc");
    ...
    it("should call method-level middleware correctly (PUT)", (done) => {
      @controller("/")
      class TestController {
        @httpPut("/", spyA, spyB, spyC) public postTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("PUT");
      ...
      agent.put("/").expect(200, "PUT", function () {
        expect(spyA.calledOnce).to.eqls(true);
        expect(spyB.calledOnce).to.eqls(true);
        expect(spyC.calledOnce).to.eqls(true);
        expect(result).to.equal("abc");
    ...
    it("should call method-level middleware correctly (PATCH)", (done) => {
      @controller("/")
      class TestController {
        @httpPatch("/", spyA, spyB, spyC) public postTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("PATCH");
      ...
      agent.patch("/").expect(200, "PATCH", function () {
        expect(spyA.calledOnce).to.eqls(true);
        expect(spyB.calledOnce).to.eqls(true);
        expect(spyC.calledOnce).to.eqls(true);
        expect(result).to.equal("abc");
    ...
    it("should call method-level middleware correctly (HEAD)", (done) => {
      @controller("/")
      class TestController {
        @httpHead("/", spyA, spyB, spyC) public postTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("HEAD");
      ...
      agent.head("/").expect(200, "HEAD", function () {
        expect(spyA.calledOnce).to.eqls(true);
        expect(spyB.calledOnce).to.eqls(true);
        expect(spyC.calledOnce).to.eqls(true);
        expect(result).to.equal("abc");
    ...
    it("should call method-level middleware correctly (DELETE)", (done) => {
      @controller("/")
      class TestController {
        @httpDelete("/", spyA, spyB, spyC) public postTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("DELETE");
      ...
      agent.delete("/").expect(200, "DELETE", function () {
        expect(spyA.calledOnce).to.eqls(true);
        expect(spyB.calledOnce).to.eqls(true);
        expect(spyC.calledOnce).to.eqls(true);
        expect(result).to.equal("abc");
    ...
    it("should call method-level middleware correctly (ALL)", (done) => {
      @controller("/")
      class TestController {
        @all("/", spyA, spyB, spyC) public postTest(
          req: express.Request,
          res: express.Response
        ) {
          res.send("ALL");
      ...
      agent.get("/").expect(200, "ALL", function () {
        expect(spyA.calledOnce).to.eqls(true);
        expect(spyB.calledOnce).to.eqls(true);
        expect(spyC.calledOnce).to.eqls(true);
        expect(result).to.equal("abc");
  ...
  describe("Parameters:", () => {
    it("should bind a method parameter to the url parameter of the web request", (done) => {
      ...
      server = new InversifyExpressServer(container);
      supertest(server.build()).get("/foo").expect(200, "foo", done);
    });
    ...
    it("should bind a method parameter to the request object", (done) => {
      ...
      server = new InversifyExpressServer(container);
      supertest(server.build()).get("/GET").expect(200, "GET", done);
    });
    ...
    it("should bind a method parameter to the response object", (done) => {
      ...
      server = new InversifyExpressServer(container);
      supertest(server.build()).get("/").expect(200, "foo", done);
    });
    ...
    it("should bind a method parameter to the request body", (done) => {
      ...
      supertest(server.build()).post("/").send(body).expect(200, body, done);
    });
    ...
    it("should bind a method parameter to a principal with null (empty) details when no AuthProvider is set.", (done) => {
      @controller("/")
      class TestController {
        @httpGet("/") public getPrincipalTest(
          @principal() userPrincipal: interfaces.Principal
        ) {
    ...
    it("should bind a method parameter to a principal with valid details when an AuthProvider is set.", (done) => {
      ...
      supertest(server.build()).get("/").expect(200, "something", done);
    });
  });
});
